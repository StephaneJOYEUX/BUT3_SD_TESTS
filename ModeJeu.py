    ...
    def verifier_validiter_navires(self, navires : set = None, taille_grille : list = None):
        ...
        for navire in navires :
            if navire.get_taille() >= min(taille_grille) :
                ...
                print("La longueur des navires ne peut pas être supérieure ou égale au nombre minimum de ligne/colonne.\n")
                numero_critere = 1
        ...
        if somme/(taille_grille[0]*taille_grille[1]) > 0.6 :
            os.system('cls')
            print("\nCritère de validité non respecté !")
            print("Le nombre de cases occupées par vos navires est trop important par rapport à la taille de la grille.")
